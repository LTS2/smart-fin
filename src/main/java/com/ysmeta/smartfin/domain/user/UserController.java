...
public class UserController {
	...
	/**
	 * UserController 생성자입니다. UserService 객체를 주입받아 초기화합니다.
	 *
	 * @param userService IUserService 인터페이스를 구현한 서비스 객체
	 */
	@Autowired
	...
	public ResponseEntity<String> registerUser(@Valid @RequestBody UserDto.CreateUserRequestDto createUserRequestDto,
		...
		// TODO: userService.registerUser(createUserRequestDto) 메서드를 호출하여 실제 회원가입 로직을 처리해야 함
		// 현재는 회원가입 완료 메시지와 함께 HTTP 상태 코드를 반환하는 임시 코드입니다.
		// userService.saveUser(createUserRequestDto.getEmail());
